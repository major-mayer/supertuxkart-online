...
then 
    ...
    then
        ...
        sleep 15 && supertuxkart --connect-now=localhost:2759 --network-ai=$stk_aikarts

    else
        echo "\$stk_serverpw is set - connect $stk_aikarts bots to server with password"
        sleep 15 && supertuxkart --connect-now=localhost:2759 --server-password=$stk_serverpw --network-ai=$stk_aikarts 
